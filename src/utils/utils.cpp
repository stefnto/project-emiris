...
void Data_point::setDistanceFromQuery(double distanceFromQuery){
  this->distanceFromQuery = distanceFromQuery;
}


double Data_point::getDistanceFromQuery() const{
...
int hFunction::operator()(const Data_item* item, const std::vector<double>* altered_coordinates){
  std::vector<float>::const_iterator it2 = v.begin();
  std::vector<double>::const_iterator it1;

  if (altered_coordinates){
    it1 = altered_coordinates->begin();

    if( altered_coordinates->size() != v.size() ) {
      std::cout <<"size2 : " << altered_coordinates->size() << std::endl;
      std::cout << "size2 : " << v.size() << std::endl;
      throw item_Exception();
    }
  }
  else if (!altered_coordinates){
    it1 = item->getCoordinates().begin();
  }
...
double EuclidianDistance(const std::vector<double>& a,const std::vector<double>& b){
  if ( a.size() != b.size() ){
    std::cout << "EuclidianDistance : Unequal sizes between vector a & b " << std::endl;
